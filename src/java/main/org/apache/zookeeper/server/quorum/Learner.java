...
public class Learner {       
    ...
    protected long registerWithLeader(int pktType) throws IOException{
		...
		if (qp.getType() == Leader.LEADERINFO) {
        	...
        	// 读取到leader的信息后，构造一个ack并发送给leader
    ...
    protected void syncWithLeader(long newLeaderZxid) throws IOException, InterruptedException{
        ...
        // leader并不是会将每一次事务操作都同步到快照文件中，而是一批事务一批事务的通过
        // 所以同步leader上的数据需要分成三种情况
        // 1. 如果learner的事务id大于快照中最大的事务id并小于leader最大的事务id，此时不需要同步快照的内容，而是增量同步
        // 2. 如果learner的事务id小于快照中的最大的事务id，则会先清空内存中的数据，再同步快照信息
        // 3. 如果learner的事务id大于leader最大的事务id，那么就会截断多余的信息
        synchronized (zk) {
