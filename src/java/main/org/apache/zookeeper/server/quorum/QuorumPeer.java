...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    private void loadDataBase() {
		...
		try {
            ...
            // zxid是一个64位的数字，它高32位是epoch用来标识leader关系是否改变，
            // 每次一个leader被选出来，它都会有一个新的epoch，标识当前属于那个leader的统治时期。
            // 低32位用于递增计数。
            // 在这里对zxid进行右移32位的处理，从而获得epoch的值
        ...
        try {
            ...
            while (running) {
                ...
                case LOOKING:
                    ...
                    break;
                case OBSERVING:
                    // 如果是观察状态，就需要去同步leader中的数据
                    try {
                        LOG.info("OBSERVING");
                        setObserver(makeObserver(logFactory));
                        // 1. 先找到leader的服务器地址
                        // 2. 通过socket连接到leader(会尝试连接5次)
                        // 3. 将observer的信息注册到leader上
